...
echo "\n\n" . __( 'Installment Plan Information', 'installment-plans-for-woo-subs' ) . "\n\n";
foreach ( $subscriptions as $subscription ) {
	...
	echo sprintf( _x( 'Installment: %s', 'in plain emails for subscription information', 'installment-plans-for-woo-subs' ), wp_strip_all_tags( $content_args['payment-detail'] ) ) . "\n";

	// translators: placeholder is the localised date of payment.
	echo sprintf( _x( 'Payment Date: %s', 'in plain emails for subscription information', 'installment-plans-for-woo-subs' ), date_i18n( wc_date_format(), $subscription->get_time( 'date_created', 'site' ) ) ) . "\n\n";

	// A header for the plan details.
	echo '--' . _x( 'Plan Details', 'in plain emails for subscription information', 'installment-plans-for-woo-subs' ) . '--' . "\n";

	// translators: placeholder is total number of installment payments.
	echo sprintf( _x( 'Total payments: %s', 'in plain emails for subscription information', 'installment-plans-for-woo-subs' ), $content_args['single-count'] ) . "\n";

	// translators: placeholder is installment type.
	echo sprintf( _x( 'Terms: %s', 'in plain emails for subscription information', 'installment-plans-for-woo-subs' ), wp_strip_all_tags( $content_args['payment-schedule'] ) ) . "\n";

	// translators: placeholder is the total amount of the subscription
	echo sprintf( _x( 'Total cost: %s', 'in plain emails for subscription information', 'installment-plans-for-woo-subs' ), wp_strip_all_tags( $content_args['total-cost'] ) ) . "\n";

	if ( $subscription->get_time( 'next_payment' ) > 0 ) {
		echo sprintf( esc_html__( 'Next payment: %s', 'installment-plans-for-woo-subs' ), esc_html( date_i18n( wc_date_format(), $subscription->get_time( 'next_payment', 'site' ) ) ) );
	} else {
		echo sprintf( esc_html__( 'Next payment: %s', 'installment-plans-for-woo-subs' ), $content_args['no-remaining'] );
	}
